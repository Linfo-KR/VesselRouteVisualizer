...
CSV 형식의 입력 파일을 받아 처리하며, 설정을 통해 유연하게 데이터 처리 방식을 변경할 수 있습니다.
...
- **모듈화된 구조**: 데이터 조회(`mapper`), 라우트 처리(`route`), 스케줄 정규화(`normalize_schedule`), 데이터 입출력(`export_data`) 등 기능별로 모듈이 분리되어 유지보수 및 확장이 용이
...
4.  **`DataMapper` 생성**: `ExportData`는 `proforma.csv` 파일을 읽어 기준 정보를 메모리에 적재한 `DataMapper` 객체를 생성합니다. (이때, 한국어 환경을 고려하여 `cp949` 인코딩을 기본으로 사용합니다.)
5.  **`RouteProcessor` 생성**: 생성된 `DataMapper` 객체를 `RouteProcessor`에 주입하여 핵심 로직을 수행할 객체를 생성합니다.
6.  **데이터 처리**: `RouteProcessor`는 `bpa_service_code.csv`의 각 행을 읽어 라우트 코드를 조합하고, `DataMapper`를 통해 최대 `max_terminals` 개수만큼 터미널 정보를 조회합니다. (이 과정에서 중복된 서비스 코드(키)가 있어도 안정적으로 조회합니다.)
7.  **스케줄 정규화**: `ExportData`는 조회된 스케줄(`Sch`) 정보를 `ScheduleNormalizer`를 이용해 표준화된 형식의 문자열로 변환합니다.
8.  **결과 생성**: 조회 및 정규화된 터미널 정보(`Name`, `Wtp`, `Sch`)를 원본 데이터에 `T1Name`, `T2Name`... 과 같은 새로운 컬럼으로 추가합니다.
9.  **파일 저장**: `ExportData`는 최종적으로 완성된 데이터프레임을 `result.csv` 파일로 저장하고, `main.py`는 처리 완료 메시지와 함께 결과 미리보기를 출력합니다.
...
`main.py` 파일 상단의 `config` 딕셔너리에서 주요 동작을 변경할 수 있습니다.
