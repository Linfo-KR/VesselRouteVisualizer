# VesselRouteVisualizer: 부산항 정기 노선 시각화 프로젝트

## 📖 개요 (Overview)

- Gemini CLI를 활용해 아이디어를 빠르게 코드로 구현
- **목적:** PPT 수작업으로 작성하던 부산항 정기 노선도를 항만 좌표와 상세 노선 정보를 기반으로 시각화하고, 웹 상에서 정기 노선 및 관련 스케줄/물량 정보를 조회 가능한 기능 구현
- **핵심 기능:**
    - 지도 기반의 노선 시각화
    - 항만 정보 및 좌표 관리(추가 / 수정 / 제거)
    - 부산항 기항 시 터미널 별 물동량 정보 조회(Proforma)
    - 상세 노선 및 선박 정보 조회
...
### 3. TB_PROFORMA (터미널 기항 정보 상세)
각 노선의 기항 순서와 터미널 상세 정보를 저장
- `term_id` (PK): 고유 ID
- `route_idx` (FK): 노선 ID
- `terminal_name`: 터미널명
- `seq`: 기항 순서
...
| `GET` | `/api/ports` | 모든 항만 목록 조회 (검색 가능) |
| `GET` | `/api/ports/{code}` | 특정 항만의 상세 정보 조회 |
| `POST` | `/api/fix-port-mismatch` | 항만 정보 오류 시 수정 요청 |
...
CSV 파일의 데이터를 데이터베이스에 적재합니다. 초기 실행 시 또는 데이터 업데이트 시 실행합니다.
...
- [x] **React 기반 Frontend 개발**
    ...
    - [ ] 노선 검색 및 정보 조회 UI -> UI 수정 필요
    - [ ] 노선 가시성 최적화(https://github.com/genthalili/searoute-py 참고, 실제 항로와 유사하게...)
- [ ] 배포 (Docker, AWS)
